...
**오염된 프로필에 설치해도 안전** — cys 마스터를 사용자 `~/.claude`(외부 터미널 체계·구 마스터지침 오염
가능)로부터 **격리**해, 어떤 프로필 상태에서 설치해도 cys가 깨끗이 부팅됩니다. (Apple Silicon arm64)

## 하이라이트

### cys 전용 CLAUDE_CONFIG_DIR 격리 (근본 사고 차단)
다른 맥에 설치 시 표준 claude의 공용 `~/.claude`에 남은 **외부 터미널 체계/구 마스터지침**이 마스터를
하이재킹해 외부 터미널 체계 체계로 헛돌다 멈추는 사고가 있었습니다. 0.2.2는 이를 근본 차단합니다.
...
- **preflight C31**: 격리 라우터 설치 확인 + 사용자 프로필의 외부 터미널 체계/구지침을 **감지·경고**
  (삭제는 사용자 선택).
