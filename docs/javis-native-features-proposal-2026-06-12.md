...
- 작성일: 2026-06-12
- 전제: 기능 parity는 노선 (a) 기능 재구현 유지 (오너 확정). 본 제안은 parity와 별개로, **자비스 시스템이 거주하는 터미널**로서의 고유 기능을 다룬다.
...
| 재부팅 후 PTY 미할당 → dispatch 불가 (외부 터미널 앱 버그) | 2026-06-06 | 높음 | T2 전체 (cysd는 구조적으로 면역 + 토폴로지 복원) |
...
- **효과**: 화면 파싱 0줄로 컨텍스트·진행 추적 해결. 기능 parity #1(set-status/progress)과 **동일 기안으로 통합** — parity와 자비스 요구가 정확히 겹치는 지점.
...
- **효과**: 리뷰어의 master 조향·디렉티브 위조 주입 경로 차단. T4-18 attest의 전제. 외부 터미널의 소켓 비밀번호 인증보다 자비스에 적합한 모델(역할 기반).
...
- **기능**: master 역할 부재 또는 무출력 N분 → UI 최상위 경보 + (설정 시) 오너 외부 알림. 조직 복원력의 마지막 구멍 봉합.
