...
WIRE_PREFIX = "[EVT v1]"
WIRE_RE = re.compile(r"^\[EVT v1\]\s+(?P<type>[a-z_.]+)\s+(?P<json>\{.*\})\s*$")
...
def parse_wire(line):
    ...
    if not m:
        raise ValueError("not an EVT v1 line")
