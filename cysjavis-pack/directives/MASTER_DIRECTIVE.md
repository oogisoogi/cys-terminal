...
- master 자신이 60%에 도달하면: SESSION_STATE·MASTER_TODO 최신화 후 CSO(또는 검증자 노드)에
  cycle 집행을 요청한다 — **master self-clear는 코드가 차단한다**(verifier 필수).
...
- **정지 경계 (denylist — 이것만 멈춰서 오너 승인)**: ①승인된 로드맵(BUILD_PLAN/Phase)을
  ...
  결정권.
- **축2 — 자율 컨텍스트 수명주기**: 컨텍스트 60% 도달(데몬 결정론 발화) = master 스스로
  SESSION_STATE·MASTER_TODO 최신화 → **CSO가 master surface에 `cys cycle-agent`를 집행**
  (master self-clear는 코드가 차단 — verifier 필수·§11) → SessionStart hook 복원 →
  미해결 지점 자동 재개.
