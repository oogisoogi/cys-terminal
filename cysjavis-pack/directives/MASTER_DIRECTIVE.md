...
- **★master 컨텍스트 clear = CSO 주도 "주인 대리" 핸드셰이크 (2026-06-18 박사님 제정 · 절대규칙)**:
  master self-clear는 **절대 금지**(자기참조 = 자기 전원 차단). master 컨텍스트 clear는 **CSO가
  "주인(박사님)을 대신하여"** 집행한다 — CSO의 `/clear`는 주인이 직접 친 것과 동일한 인가 행위이며
  ...
  2. **CSO 시점 판단·통보(개시 주체 = CSO)**: CSO가 시점을 판단(60% 신호 + 안전지점 = 게이트/커밋
     중간 아님·박사님 실시간 입력 중 아님)하여 master에 "[CSO·주인 대신] clear 시점 — 세션 재개
     준비하라"를 통보한다.
  ...
  6. **🔴 무응답 정책(박사님 2026-06-18 결정 = 독립검증 후 조건부 집행)**: master가 통보 후
     ...
     ②낡음(미저장 위험) → clear 금지·박사님께 escalation. 무한 대기·맹목 force-clear 없음
     (eval-driven 독립 재유도 원칙과 정합). **CSO 부재 / AUTOPILOT_PAUSED / 박사님 실시간 입력 중 =
...
- **축2 — 자율 컨텍스트 수명주기 (★CSO 주도 주인 대리 clear)**: 컨텍스트 60% 도달(데몬 결정론
  ...
  무응답 시 CSO 독립검증 후 조건부 집행(신선=집행·낡음=박사님 escalation). 6단계 전문·무응답
  정책은 **§11**(컨텍스트 사이클) 참조.
