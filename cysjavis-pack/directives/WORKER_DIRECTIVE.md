...
- **todo 필수**: 받은 task는 반드시 todo로 분해하고 세부 완료마다 갱신한다. todo는
  **pack의 round** — `~/.cys/pack/round/<역할>_TODO.md`(CYS_PACK_DIR 설정 시 그 하위) —
  에 영속화한다(세션 재시작 복원용 · 진행% 집계기의 기본 스캔 경로 — 작업 폴더
...
- **진행% 가시성**: master는 5분마다 너의 `~/.cys/pack/round/WORKER_TODO.md` 체크박스
...
- **작업 단위마다 `cys set-status --state <s> --context <추정%>`로 컨텍스트 사용률을
  ...
  수치 보고가 트리거다.
- 재시작·clear 후에는 자신의 todo md(`~/.cys/pack/round/WORKER_TODO.md`)부터 읽고 이어간다.
